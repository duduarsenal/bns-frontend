...
export default function EncomendasCard(props: any){

    const { morador, cdrastreio, status, dtchegada, dtretirada, recebedor, className } = props;
    ...
    return (
        <section className={`${className} w-full flex flex-col gap-1 justify-center font-normal text-[#4C9773] cursor-pointer`}>
            <div className="flex items-center justify-between h-max w-full border-[3px] border-[#4C9773] rounded-lg min-h-[70px] px-2" onClick={() => setDropDown(!dropDown)}>
            ...
            <div className={`${!dropDown && '-mt-[70px] !h-0 opacity-0 pointer-events-none'} visible mt-0 transition-all duration-[400ms] h-[50px] bg-[#124C3850] rounded-lg flex items-center justify-around px-4 cursor-default mx-4`}>
                ...
                <div className="flex items-center justify-center gap-2">
                    ...
                    <Input
                        ...
                        value={recebedor || recebedorUpdt}
                        onChange={(e) => setRecebedorUpdt(e.target.value)}
                        className="rounded-md border-2 border-b-2 !border-[#124C3890]"
                     />
                </div>
                <Button text="Salvar" className="bg-[#32775fbc] text-[#11111190] px-6 py-1 rounded-md font-semibold hover:scale-[1.05] transition-all"/>
            </div>
        </section>
    )
}